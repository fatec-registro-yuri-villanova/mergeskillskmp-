package com.fatec.merge_skills_kmp.services

import com.fatec.merge_skills_kmp.domain.models.*
...
class SeedService(private val client: SupabaseClient) {
    ...
    suspend fun executeSeed(): Int {
        ...
        // 1. Seed Users (Fallback auth data)
        seedUser()

        // 2. Fetch existing courses to resolve relationships when UPSERTING
        ...
        // Filter out those that we already have if we don't want to duplicate records in case of Upsert missing constraints
        val coursesToUpsert = coursesPayload.filter { !existingTitles.contains(it.title) }

        val allCourses = existingCourses.toMutableList()

        if (coursesToUpsert.isNotEmpty()) {
            // Using UPSERT as requested
            val insertedCourses = client.from("courses").upsert(coursesToUpsert) { select() }.decodeList<Course>()
            allCourses.addAll(insertedCourses)
            newCoursesInserted = insertedCourses.size
        }

        // 3. Process each course via Upsert
        allCourses.forEach { course ->
            when (course.title) {
                "Java Icaro" -> seedCourse1(course.id)
                "Kotlin" -> seedCourse2(course.id)
                "Python" -> seedCourse3(course.id)
                "TypeScript" -> seedCourse4(course.id)
            }
        }

        return newCoursesInserted
    }
    ...
    private suspend fun seedCourse1(courseId: Int) {
        ...
        val lessonsToUpsert = lessonsPayload.filter { newLesson -> existingLessons.none { it.order == newLesson.order } }
        val allLessons = existingLessons.toMutableList()
        
        if (lessonsToUpsert.isNotEmpty()) {
            val insertedLessons = client.from("lessons").upsert(lessonsToUpsert) { select() }.decodeList<Lesson>()
        ...
        if (existingQ1.isEmpty()) {
            upsertQuestionsPolymorphic(listOf(
    ...
    private suspend fun seedCourse2(courseId: Int) {
        val existingLessons = try { client.from("lessons").select { filter { eq("course_id", courseId) } }.decodeList<Lesson>() } catch(e: Exception) { emptyList() }
        val lessonsPayload = listOf(
    ...
    private suspend fun seedCourse3(courseId: Int) {
        val existingLessons = try { client.from("lessons").select { filter { eq("course_id", courseId) } }.decodeList<Lesson>() } catch(e: Exception) { emptyList() }
        val lessonsPayload = listOf(
    ...
    private suspend fun seedCourse4(courseId: Int) {
        val existingLessons = try { client.from("lessons").select { filter { eq("course_id", courseId) } }.decodeList<Lesson>() } catch(e: Exception) { emptyList() }
        val lessonsPayload = listOf(
